...
' ==========================================================
' NOUVEAU_SIREN_V1_CORE
' Extrait vérifié depuis l'archive complète du moteur lexical.
' Ce fichier présente le coeur fonctionnel du projet et n'est
' pas destiné à remplacer l'archive complète comme module autonome.
' ==========================================================
...
Public Sub NOUVEAU_SIREN_V1()
    ...
    If TypeName(Selection) <> "Range" Then
        MsgBox "Selectionne les cellules a analyser puis relance NOUVEAU_SIREN_V1.", vbExclamation
        Exit Sub
    End If
    ...
    If lignes.Count = 0 Then
        MsgBox "Aucune cellule exploitable dans la selection.", vbExclamation
        Exit Sub
    End If
...
FinPropre:
    ...
    MsgBox "NOUVEAU SIREN V1 termine." & vbCrLf & _
           "Lignes selectionnees traitees : " & traitees & vbCrLf & _
           "Nouveaux SIREN trouves : " & trouves & vbCrLf & _
...
FinAvecErreur:
    ...
    Application.EnableEvents = oldEnableEvents
    Application.StatusBar = oldStatusBar
    MsgBox "NOUVEAU SIREN V1 s'est arrete : " & Err.Description, vbCritical
End Sub
...
' L'archive complète contient également :
' - analyse détaillée des annonces BODACC ;
' - moteur lexical des événements juridiques ;
' - fusion / TUP / scission / apport partiel / cession ;
' - procédures collectives et liquidations ;
' - extraction de sociétés bénéficiaires ;
' - recherche du SIREN cible ;
' - scoring par date et contrôles de cohérence ;
' - fonctions HTTP, JSON, normalisation et cache.
